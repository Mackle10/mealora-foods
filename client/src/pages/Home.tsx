...
export default function Home() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<"cash_on_delivery" | "mtn_momo" | "airtel_money" | "card">("mtn_momo");
  const [mobileMoneyPhone, setMobileMoneyPhone] = useState("");
  const [specialInstructions, setSpecialInstructions] = useState("");
  const [paymentProcessing, setPaymentProcessing] = useState(false);
  const [paymentSuccess, setPaymentSuccess] = useState<{ orderNumber: string; mobileMoneyReference?: string; paymentMethod: string } | null>(null);
  ...
  const reviewsQuery = trpc.reviews.list.useQuery({ restaurantId: menuRestaurant?.id ?? 0 }, { enabled: Boolean(menuRestaurant) });
  ...
  const checkout = async () => {
    ...
    if ((paymentMethod === "mtn_momo" || paymentMethod === "airtel_money") && mobileMoneyPhone.trim().length < 7) {
      toast("Add your Mobile Money number", { description: "Use the number that should receive the payment prompt." });
      return;
    }
    try {
      setPaymentProcessing(true);
      const result = await createOrder.mutateAsync({ restaurantId, deliveryAddress: location, specialInstructions: specialInstructions.trim() || undefined, paymentMethod, mobileMoneyPhone: mobileMoneyPhone.trim() || undefined, items: cart.map((entry) => ({ menuItemId: entry.item.id, quantity: entry.quantity })) });
      setCart([]);
      setMenuRestaurant(null);
      setPaymentSuccess({ orderNumber: result.orderNumber, mobileMoneyReference: result.mobileMoneyReference, paymentMethod: result.paymentMethod });
      toast.success(`Order ${result.orderNumber} created`, { description: result.paymentMethod === "cash_on_delivery" ? "Pay the courier when your order arrives." : `${paymentMethod === "mtn_momo" ? "MTN MoMo" : paymentMethod === "airtel_money" ? "Airtel Money" : "Card"} payment request ${result.mobileMoneyReference ? `#${result.mobileMoneyReference}` : "is ready"}.` });
    } catch (error) {
  ...
  const saveRestaurant = async (restaurant: Restaurant) => {
    ...
    <header className="market-header"><button className="market-brand" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}><span className="brand-orbit"><Globe2 size={17} /></span><span>MEALORA<small>FOODS</small></span></button><nav className="market-nav"><button onClick={() => document.getElementById("cities")?.scrollIntoView({ behavior: "smooth" })}>Discover</button><button onClick={() => document.getElementById("tracking")?.scrollIntoView({ behavior: "smooth" })}>Track an order</button><a href="/partners">For partners</a></nav><div className="market-actions"><button className="delivery-location" onClick={setDeliveryLocation}><MapPin size={14} />{location}<ChevronDown size={13} /></button>{isAuthenticated ? <button className="profile-chip" onClick={() => setProfileOpen(!profileOpen)}><span>{user?.name?.slice(0, 1).toUpperCase() ?? "M"}</span>{user?.name?.split(" ")[0] ?? "Profile"}</button> : <a className="login-button" href="/auth">Log in</a>}<button className="bag-button" onClick={() => setProfileOpen(!profileOpen)}><ShoppingBag size={17} /><b>{cartCount}</b></button><button className="mobile-menu" aria-label="Open navigation" onClick={() => setMobileNavOpen(true)}><Menu size={20} /></button></div></header>
    {profileOpen && <><button className="soft-backdrop" aria-label="Close account menu" onClick={() => setProfileOpen(false)} /><div className="profile-popover"><div className="profile-popover-head"><div className="profile-avatar">{user?.name?.slice(0, 1).toUpperCase() ?? "M"}</div><div><strong>{user?.name ?? "Your Mealora account"}</strong><small>{user?.email ?? "Sign in to unlock orders and saved places"}</small></div></div>{isAuthenticated ? <><a href="/profile">Open profile & order history <ArrowRight size={14} /></a><button onClick={() => void logout().then(() => { setProfileOpen(false); toast.success("You are logged out"); }).catch(() => toast.error("Could not log out"))}>Log out</button></> : <a href="/auth">Log in to unlock orders <ArrowRight size={14} /></a>}<button onClick={() => setProfileOpen(false)}>Close</button></div></>}

    <>{mobileNavOpen && <><button className="nav-backdrop" aria-label="Close navigation" onClick={() => setMobileNavOpen(false)} /><aside className="mobile-drawer" aria-label="Mobile navigation"><div className="mobile-drawer-head"><span>MEALORA <em>UGANDA</em></span><button aria-label="Close navigation" onClick={() => setMobileNavOpen(false)}><X size={19} /></button></div><button onClick={() => { setMobileNavOpen(false); document.getElementById("cities")?.scrollIntoView({ behavior: "smooth" }); }}>Discover places <ArrowRight size={16} /></button><button onClick={() => { setMobileNavOpen(false); document.getElementById("tracking")?.scrollIntoView({ behavior: "smooth" }); }}>Track an order <ArrowRight size={16} /></button><a href="/partners" onClick={() => setMobileNavOpen(false)}>For partners <ArrowRight size={16} /></a><div className="mobile-drawer-account">{isAuthenticated ? <><span>Signed in as {user?.name?.split(" ")[0] ?? "you"}</span><button onClick={() => void logout().then(() => { setMobileNavOpen(false); toast.success("You are logged out"); }).catch(() => toast.error("Could not log out"))}>Log out</button></> : <a href="/auth" onClick={() => setMobileNavOpen(false)}>Log in to Mealora <ArrowRight size={16} /></a>}</div></aside></>}</>
    ...
    {menuRestaurant && <div className="modal-backdrop" onClick={() => !paymentProcessing && setMenuRestaurant(null)}><section className="menu-modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" disabled={paymentProcessing} onClick={() => setMenuRestaurant(null)}><X size={17} /></button><div className="menu-modal-head"><img src={menuRestaurant.imageUrl} alt={menuRestaurant.name} /><div><span>{menuRestaurant.cuisine}</span><h2>{menuRestaurant.name}</h2><p>{menuRestaurant.description}</p></div></div><div className="menu-list">{menu.length ? menu.map((item) => <div className="menu-item" key={item.id}><img src={item.imageUrl} alt={item.name} /><div><strong>{item.name}</strong><p>{item.description}</p><span>{formatMoney(item.priceCents, item.currency)}</span></div><button onClick={() => addToCart(item)}><Plus size={15} /></button></div>) : <div className="menu-loading">Loading the live menu…</div>}</div><div className="menu-reviews"><div className="menu-reviews-head"><span>RECENT REVIEWS</span><small>{reviewsQuery.data?.length ?? 0} visible</small></div>{reviewsQuery.data?.length ? reviewsQuery.data.slice(0, 4).map((review) => <article className="menu-review" key={review.id}><div><strong>{"★".repeat(review.rating)}{"☆".repeat(5 - review.rating)}</strong><p>{review.comment || "Customer left a rating without a comment."}</p>{review.reply && <small>Restaurant reply: {review.reply}</small>}</div><ReportReviewButton reviewId={review.id} /></article>) : <p className="menu-reviews-empty">No reviews yet. Be the first to share your experience after delivery.</p>}</div><div className="checkout-payment"><span>PAY WITH</span><select disabled={paymentProcessing} value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value as typeof paymentMethod)}><option value="mtn_momo">MTN MoMo</option><option value="airtel_money">Airtel Money</option><option value="cash_on_delivery">Cash on delivery</option><option value="card">Card</option></select>{paymentMethod === "mtn_momo" || paymentMethod === "airtel_money" ? <input disabled={paymentProcessing} className="mobile-money-phone" type="tel" value={mobileMoneyPhone} onChange={(event) => setMobileMoneyPhone(event.target.value)} placeholder="Mobile Money number · +256 7XX XXX XXX" /> : null}<label className="checkout-instructions">Special instructions<textarea disabled={paymentProcessing} value={specialInstructions} onChange={(event) => setSpecialInstructions(event.target.value)} placeholder="E.g. less spicy, call at the gate…" maxLength={500} /></label><small>{paymentMethod === "mtn_momo" || paymentMethod === "airtel_money" ? "You’ll receive a payment prompt on this number after checkout." : paymentMethod === "cash_on_delivery" ? "Pay the courier in cash when your order arrives." : "Card payments will open when Stripe keys are configured."}</small></div><div className="menu-modal-foot"><span>{cartCount} items · {formatMoney(cartTotal)}</span><button disabled={paymentProcessing} onClick={() => void checkout()}>{paymentProcessing ? <><span className="button-spinner" /> Processing…</> : <>Checkout <ArrowRight size={15} /></>}</button></div>{paymentProcessing && <div className="payment-processing"><span className="payment-loader" /><strong>Waiting for your payment</strong><p>Check your phone and approve the {paymentMethod === "mtn_momo" ? "MTN MoMo" : "Airtel Money"} prompt.</p></div>}</section></div>}
    {paymentSuccess && <div className="modal-backdrop"><section className="payment-success-modal"><div className="success-mark"><Check size={25} /></div><span className="dashboard-kicker">ORDER CONFIRMED</span><h2>You’re all set.</h2><p>{paymentSuccess.paymentMethod === "cash_on_delivery" ? "Your order is being prepared. Pay the courier on arrival." : "Your payment request was created. We’ll text you when payment is confirmed."}</p><strong>{paymentSuccess.orderNumber}</strong>{paymentSuccess.mobileMoneyReference && <small>Payment reference · {paymentSuccess.mobileMoneyReference}</small>}<button onClick={() => setPaymentSuccess(null)}>Continue exploring <ArrowRight size={15} /></button></section></div>}
